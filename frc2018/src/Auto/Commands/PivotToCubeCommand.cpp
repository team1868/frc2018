...
//	Profiler profiler(robot_, "Align With Peg Init");
	...
	try {
		...
		subscriber_->connect("tcp://10.18.68.97:5563");	// MAKE SURE RIGHT IP
		int confl = 1;
		...
		case (kPivotToAngleInit) :
			...
			if (diffInAngle > 2.0 || (diffInAngle == 0.0 && desiredPivotDeltaAngle_ == 0.0)) {
				...
				if (pivotCommand_ == NULL) {
					...
					fflush(stdout);
					pivotCommand_ = new PivotCommand(robot_, -desiredPivotDeltaAngle_, true, navXSource_);
...
//@todo change drive straight code here
			ReadFromJetson();
